...
//				assert(false) : "Verbose flag is currently static final; must be recompiled to change.";
			...
			}else if(a.equals("window") || a.equals("windowlen")){
				windowLen=Integer.parseInt(b);
			}else if(a.equals("windowcount") || a.equals("windowlimit")){
...
//			ECC_PINCER=ECC_TAIL=ECC_ALL=false;
//			ECC_REASSEMBLE=true;
		...
		private void processRead(Read r){
			...
			if(ecc || MARK_BAD_BASES>0){
				...
				if(detected>0){
					...
					if(corrected==detected || (corrected>0 && countErrors(countList)==0)){
...
//	}
	...
	public final int countErrors(IntList counts){
		int possibleErrors=0;
		for(int i=1; i<counts.size; i++){
			final int a=counts.get(i-1), b=counts.get(i);
			if(isErrorBidirectional(a, b)){
				possibleErrors++;
				if(a>b){i+=kbig;}
			}
		}
	...
	public final int reassemble(final byte[] bases, final byte[] quals, final int[] rightCounts, final IntList counts, final IntList counts2, 
			final ErrorTracker tracker, final int errorExtension, final ByteBuilder bb, final ByteBuilder bb2, final Kmer kmer, final Kmer regenKmer){
		...
		int corrected=reassemble_pass(bases, quals, fromLeft, fromRight, rightCounts, counts, counts2, tracker, errorExtension, kmer, regenKmer);
		...
		while(correctedIncr>0 && uncorrected>0){
			tracker.detectedReassemble-=uncorrected;
			detected0=tracker.detectedReassemble;
			correctedIncr=reassemble_pass(bases, quals, fromLeft, fromRight, rightCounts, counts, counts2, tracker, errorExtension, kmer, regenKmer);
	...
	public final int reassemble_pass(final byte[] bases, final byte[] quals, final ByteBuilder fromLeft, final ByteBuilder fromRight, 
			...
			final Kmer kmer, final Kmer kmer2){
		if(bases.length<kbig+1+deadZone){return 0;}
...
//		System.err.println();
		...
		int clearedLeft=clearWindow2(fromLeft, windowLen, windowCount);
		fromRight.reverseInPlace();
		int clearedRight=clearWindow2(fromRight, windowLen, windowCount);
		fromRight.reverseInPlace();
...
//				}
			...
			if(b!=c && i>=kbig && i<bases.length-kbig){d=a;}//Clause to force Inner mode in the middle
		...
		{
			tracker.detectedReassemble+=detectedInner+detectedOuter;
	...
	//Replaced by clearWindow2
	private int clearWindow(final ByteBuilder bb, final int window, final int limit, final BitSet bs){
		...
		for(int i=0, count=0; i<len; i++){
			...
			if(b!=0){
				...
				count++;
				if(count>limit){
					bs.set(Tools.max(0, i-window), i+1);
				}
			}
			if(i>=window && array[i-window]>0){count--;}
		}
		if(!bs.isEmpty()){
	...
	private int clearWindow2(final ByteBuilder bb, final int window, final int limit){
		final int len=bb.length;
		final byte[] array=bb.array;
		...
		for(int i=0, count=0; i<len; i++){
			...
			if(b!=0){
				count++;
				if(count>limit){
//					Arrays.fill(array, Tools.max(0, i-window), bb.length(), (byte)0);
...
//		assert(toMark==0 && flag) : "toMark="+toMark+"card="+card+"len="+bases.length+"\n"+bs+"\n"+new String(bases)+"\n"+counts+"\nminCount="+minCount+"\n";
	...
	protected final boolean isErrorBidirectional(final int a, final int b){
		return isError(Tools.max(a, b), Tools.min(a, b));
	}
...
//	int ways; //MUST be set by subclass
	...
	protected int windowLen=10;
	protected int windowCount=2;

	protected byte qIncreasePincer=8;
	protected byte qMinPincer=26;
	protected byte qMaxPincer=36;
	
	protected byte qIncreaseTail=4;
	protected byte qMinTail=22;
	protected byte qMaxTail=32;
