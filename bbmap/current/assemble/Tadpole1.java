...
//						System.err.println("depth="+depth+", kmer="+toText(kmer));
	...
	public int errorCorrect(Read r, final int[] leftCounts, final int[] rightCounts, LongList kmers, IntList counts, IntList counts2, 
		...
		final int possibleErrors=tracker.suspected=countErrors(counts);
...
//			if(ECC_PINCER && tracker!=null && tracker.detected>correctedPincer){start=start-k;}
		...
		if(ECC_REASSEMBLE){
			...
			if((correctedPincer<1 && correctedTail<1) || countErrors(counts)>0){
				correctedReassemble=reassemble(bases, quals, rightCounts, counts, counts2, tracker, errorExtensionPincer, bb, bb2, null, null);
			}
...
//		assert(detectedArray==null || (correctedPincer==tracker.1] && correctedTail==tracker.2])) : correctedPincer+", "+correctedTail+", "+Arrays.toString(detectedArray);
//		if(ECC_PINCER && correctedTail>0){
//			valid=fillKmers(bases, kmers);
//			counts.reverse();
//			correctedPincer+=errorCorrectPincer(bases, quals, leftCounts, rightCounts, kmers, counts, bb, tracker, errorExtensionPincer);
//		}
		assert(correctedPincer+correctedTail+correctedReassemble==tracker.corrected())
			: correctedPincer+", "+correctedTail+", "+correctedReassemble+", "+tracker;
	
		return correctedPincer+correctedTail+correctedReassemble+correctedBrute;
	}
